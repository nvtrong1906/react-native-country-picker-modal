...
const CountryItem = (props) => {
    ...
    const { country, onSelect, withFlag, withEmoji, withCallingCode, withCurrency, callingCode } = props;
    const extraContent = [];
    if (withCallingCode &&
        callingCode) {
        extraContent.push(`+${callingCode}`);
    }
    if (withCurrency && country.currency && country.currency.length > 0) {
        extraContent.push(country.currency.join('|'));
    }
    return (React.createElement(TouchableOpacity, Object.assign({ key: country.cca2, testID: `country-selector-${country.cca2}`, onPress: () => onSelect({
        ...country, callingCode: callingCode}) }, { activeOpacity }),
...
const renderItem = (props) => ({ item: country, }) => {
    return country.callingCode.map((callingCode) => (
            React.createElement(MemoCountryItem, Object.assign({}, { country, callingCode, ...props }))
        )
    )
};
